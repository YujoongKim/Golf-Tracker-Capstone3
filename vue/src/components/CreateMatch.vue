<template>
  <div id="create-match">
    ...
    <form class="new-match"  > <!-- need to add v-on:submit.prevent="addNewScore" except addnewmatch method -->
      ...
      <select name="courseList" id="courseList" v-model="singleLeagueList">
...
<label for="score">Match name</label>
      <input class="all-labels" placeholder="Enter your match name" />
      <br>
      ...
      <select name="userList" id="userList" :multiple="true" v-model="singleLeagueList">
        ...
        <option v-for="member in singleLeagueList" :value="member.name" :key="member.id">{{member.name}}</option><br>
      </select>

      <label for="score">Tee Time:</label>
      <input class="all-labels" placeholder="Enter this format:YYYY-MM-DD HH:MM " /><br>

      

      <button id="MatchButton">Add New Match</button> <br>
